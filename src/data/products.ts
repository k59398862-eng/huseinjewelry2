...
export const getProducts = (): Product[] => {
  try {
    ...
    // Create a map of default products by id for quick lookup
    const defaultProductsMap = new Map(defaultProducts.map(p => [p.id, p]));

    // Filter out custom products that have the same id as default products
    // This ensures defaultProducts always take precedence
    const filteredCustomProducts = customProducts.filter(p => !defaultProductsMap.has(p.id));

    const all = [...defaultProducts, ...filteredCustomProducts];
